...
#
## Post installation script for vitunix x86_64
#
...
_remove_unwanted_packages() {
    local _packages_to_remove=('vitunix-welcome'
							   'vitunix-calamares'
							   'calamares'
							   'archinstall'
...
_perform_various_stuff() {
	...
	# Setting bspwm as default session #
	sddm_state='/var/lib/sddm/state.conf'
	if [[ -e "$sddm_state" ]]; then
		sed -i -e "s/User=.*/User=$new_user/g" "$sddm_state"
		sed -i -e 's#Session=.*#Session=/usr/share/xsessions/bspwm.desktop#g' "$sddm_state"
	fi
  ...
  #  [[ -x `which vitunix-hooks-runner` ]] && vitunix-hooks-runner
